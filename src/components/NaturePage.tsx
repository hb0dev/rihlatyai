...
import { Mountain, ArrowRight, MapPin, Loader2, Star } from 'lucide-react';
...
export function NaturePage({ language, onNavigateBack, onSelectPlace }: NaturePageProps) {
  ...
  const [selectedPlace, setSelectedPlace] = useState<NaturePlace | null>(null);

  const translations = {
    ar: {
      title: 'الطبيعة',
      subtitle: 'استكشف المناظر الطبيعية الخلابة',
      loading: 'جاري البحث...',
      noResults: 'لم يتم العثور على أماكن طبيعية قريبة',
      km: 'كم',
      back: 'رجوع'
    },
    fr: {
      title: 'Nature',
      subtitle: 'Explorez les paysages naturels magnifiques',
      loading: 'Recherche en cours...',
      noResults: 'Aucun lieu naturel trouvé à proximité',
      km: 'km',
      back: 'Retour'
    },
    en: {
      title: 'Nature',
      subtitle: 'Explore the stunning natural landscapes',
      loading: 'Searching...',
      noResults: 'No natural places found nearby',
      km: 'km',
      back: 'Back'
    }
  };
  ...
  const fetchPlaces = async () => {
    ...
    try {
      const response = await fetch(
        `${WORKER_URL}/places?categories=natural,national_park,leisure.park&filter=circle:${userLocation.lng},${userLocation.lat},100000&bias=proximity:${userLocation.lng},${userLocation.lat}&limit=20`
      );

      const data = await response.json();

      if (data.features) {
        const placesList: NaturePlace[] = data.features.map((feature: any) => {
          ...
          return {
            name: props.name || props.address_line1 || 'Natural Place',
            lat: coords[1],
            lng: coords[0],
            distance: distance,
      ...
      console.error('Error fetching nature places:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePlaceClick = (place: NaturePlace) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0f1419] pb-24">
      {/* Header */}
      <div className="bg-gradient-to-br from-emerald-500 to-green-500 px-5 pt-12 pb-8 rounded-b-3xl">
        <motion.button
        ...
        {showLocationPrompt ? (
          ...
          <div className="flex flex-col items-center justify-center py-20">
            <div className="bg-white dark:bg-dark-card p-6 rounded-2xl shadow-elevation-1 border border-gray-100/80 dark:border-gray-700/30">
              <Loader2 className="w-10 h-10 text-emerald-500 dark:text-emerald-400 animate-spin mb-3 mx-auto" />
          ...
          <div className="flex flex-col items-center justify-center py-20">
            <div className="bg-white dark:bg-dark-card p-8 rounded-2xl shadow-elevation-1 border border-gray-100/80 dark:border-gray-700/30">
              <Mountain className="w-14 h-14 text-gray-200 dark:text-gray-700 mb-3 mx-auto" />
            ...
            {places.map((place, index) => (
              ...
              >
                <div className="flex items-start gap-3">
                  <div className="bg-emerald-50 dark:bg-emerald-900/20 p-2.5 rounded-2xl">
                    <Mountain className="w-6 h-6 text-emerald-500 dark:text-emerald-400" strokeWidth={2} />
                  </div>
