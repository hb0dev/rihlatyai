...
import { UtensilsCrossed, ArrowRight, MapPin, Loader2, Star } from 'lucide-react';
...
export function EatPage({ language, onNavigateBack, onSelectPlace }: EatPageProps) {
  const { userLocation, isLoading: locationLoading, requestLocationPermission } = useLocation();
  const [places, setPlaces] = useState<EatPlace[]>([]);
  const [loading, setLoading] = useState(false);
  const [requestingLocation, setRequestingLocation] = useState(false);
  const [selectedPlace, setSelectedPlace] = useState<EatPlace | null>(null);

  const translations = {
    ar: {
      title: 'المطاعم',
      subtitle: 'اكتشف أفضل أماكن الطعام القريبة',
      loading: 'جاري البحث...',
      noResults: 'لم يتم العثور على مطاعم قريبة',
      km: 'كم',
      back: 'رجوع'
    },
    fr: {
      title: 'Restaurants',
      subtitle: 'Découvrez les meilleurs restaurants à proximité',
      loading: 'Recherche en cours...',
      noResults: 'Aucun restaurant trouvé à proximité',
      km: 'km',
      back: 'Retour'
    },
    en: {
      title: 'Restaurants',
      subtitle: 'Discover the best places to eat nearby',
      loading: 'Searching...',
  ...
  const fetchPlaces = async () => {
    ...
    try {
      const response = await fetch(
        `${WORKER_URL}/places?categories=catering.restaurant,catering.fast_food,catering.cafe&filter=circle:${userLocation.lng},${userLocation.lat},100000&bias=proximity:${userLocation.lng},${userLocation.lat}&limit=20`
      );

      const data = await response.json();

      if (data.features) {
        const placesList: EatPlace[] = data.features.map((feature: any) => {
          ...
          return {
            name: props.name || props.address_line1 || 'Restaurant',
            lat: coords[1],
            lng: coords[0],
            distance: distance,
  ...
  const handlePlaceClick = (place: EatPlace) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0f1419] pb-24">
      {/* Header */}
      <div className="bg-gradient-to-br from-rose-500 to-red-500 px-5 pt-12 pb-8 rounded-b-3xl">
        <motion.button
        ...
        {showLocationPrompt ? (
          ...
          <div className="flex flex-col items-center justify-center py-20">
            <div className="bg-white dark:bg-dark-card p-6 rounded-2xl shadow-elevation-1 border border-gray-100/80 dark:border-gray-700/30">
              <Loader2 className="w-10 h-10 text-rose-500 dark:text-rose-400 animate-spin mb-3 mx-auto" />
          ...
          <div className="flex flex-col items-center justify-center py-20">
            <div className="bg-white dark:bg-dark-card p-8 rounded-2xl shadow-elevation-1 border border-gray-100/80 dark:border-gray-700/30">
              <UtensilsCrossed className="w-14 h-14 text-gray-200 dark:text-gray-700 mb-3 mx-auto" />
            ...
            {places.map((place, index) => (
              ...
              >
                <div className="flex items-start gap-3">
                  <div className="bg-rose-50 dark:bg-rose-900/20 p-2.5 rounded-2xl">
                    <UtensilsCrossed className="w-6 h-6 text-rose-500 dark:text-rose-400" strokeWidth={2} />
                  </div>
